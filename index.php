...
if ($con == false) {
  ...
  $posts_sql = 'SELECT p.title AS title, text_content, avatar, video_src, image_src, page_src, quote_author, u.name AS user_name, ct.class_name AS content_type
  ...
  if (!$content_types_result || !$posts_result) {
    ...
    $main_content = include_template('main.php', ['posts' => $posts, 'content_types' => $content_types]);
